...
public class PagableInterceptor implements Interceptor {
	...
	protected void buildPaginationSql(BoundSql boundSql, Pagination<?> pagination) {
		String dialectPro = (String)properties.get("dialect");
		DialectType dialect = null;
		if("MYSQL".equals(dialectPro)){
			dialect = DialectType.MYSQL;
		}else{
			dialect = DialectType.ORACLE;
		}
		String paginationSql = DialectFactory.getDialect(dialect).buildPaginationSql(boundSql.getSql(),
	...
	protected String buildGetCountSql(BoundSql boundSql) {
		return "select count(1) from ( " + boundSql.getSql() + " ) as _t";
	}
